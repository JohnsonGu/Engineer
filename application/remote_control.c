...
void remote_control_init(void)
{
		...
		HAL_UART_Receive_DMA(&huart1,sbus_rx_buffer,36);
}
/**
...
/**
  * @brief          DMA空闲中断判断数据长度合法性，如果接收了18个字节则将原视数据拆分到结构体中
  * @retval         none
  */
...
void Usart1Receive_IDLE(void)
{
	...
	uint8_t data_length  = 36 - __HAL_DMA_GET_COUNTER(&hdma_usart1_rx);  //36-剩余空间 得到已经接收的数量
	if( data_length == 18 )
	{
			sbus_to_rc(sbus_rx_buffer, &rc_ctrl);
			Latest_Remote_Control_Pack_Time = HAL_GetTick();
	}
	memset(sbus_rx_buffer,0,data_length);
	data_length = 0;
	HAL_UART_Receive_DMA(&huart1,sbus_rx_buffer,36);
}
